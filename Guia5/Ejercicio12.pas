  ...
  reset(arch);
   while not eof (arch) do
           begin
              ...
              i:=0;
              read(arch,car);
              while car<>'' do  {Leo caracter a caracter hasta encontrar un espacio}
                      begin
                         if Especial(car)then {Identifico si es especial o no}
                            i:=i+1
                         else
                            begin
                              pal:=pal+car; {Si no es especial voy sumando los caracteres para armar la clave}
                            end;
                             read(arch,car);
                      end;
              if i<=3 then {Verifico que la cantidad de caracteres especiales sea menos o igual a 3}
                 begin
                 read(arch,a,b,c,pr);
                   ...
                   if pr<>car then    {Verifico que la ultima letra de la clave no sea la misma que el caracter leido}
                       begin
                        k:=k+1;
                        vecClave[k]:=pal;
                        vecProm[k]:=prom(a,b,c);  {Asigno todas las componentes del vector}
                        vecCar[k]:=pr;
                       end;
